...
class VideoAnalysisServiceTest {
    ...
    VideoAnalysisService pipeline; AudioPrefilterPreparationService prep; TextScreeningService text;
    ...
    @BeforeEach void setup() throws Exception {
        ...
        pipeline = new VideoAnalysisService(prep, text, segments, settings, tasks, textCalls, rows, rag, json);
    ...
    @Test void databaseSchedulerRunsUnifiedPipelineAndPersistsOutputBeforeTerminal() throws Exception {
        screening(true);
        task.setAttemptNo(0); task.setStatus("RUNNING"); task.setVideoUrl("source");
        when(tasks.selectOne(any())).thenReturn(task);
        when(tasks.storeOutput(eq("task"),eq(0),anyString(),any())).thenReturn(1);
        var repository=mock(com.videoai.worker.scheduler.TaskDispatchRepository.class);
        when(repository.leaseSeconds()).thenReturn(90);
        when(repository.candidates(anyInt())).thenReturn(List.of(new com.videoai.worker.scheduler.TaskDispatchRepository.Candidate("task",0)));
        when(repository.claim(any())).thenReturn(new com.videoai.worker.scheduler.TaskDispatchRepository.Lease("task",0,"owner")).thenReturn(null);
        var finished=new CountDownLatch(1);
        when(repository.finish(any(),eq("SUCCEEDED"),isNull())).thenAnswer(i->{finished.countDown();return true;});
        try(var scheduler=new com.videoai.worker.scheduler.DatabaseTaskScheduler(repository,pipeline,1)) {
            scheduler.start(); assertTrue(finished.await(3,TimeUnit.SECONDS));
            var order=inOrder(tasks,repository);
            order.verify(tasks).storeOutput(eq("task"),eq(0),contains("不能据此认定"),eq(0L));
            order.verify(repository).finish(any(),eq("SUCCEEDED"),isNull());
        }
        assertNull(ExecutionOwnership.current());
    }
    ...
    @Test void expiredBudgetStopsBeforePreparationAndUnknownTokensStayNull() {
        ...
        var usage = new VideoAnalysisService.TokenUsage(); usage.add(null); assertNull(usage.total());
    }
}
